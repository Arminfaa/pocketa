    ...
    >
      <div className="flex items-start justify-between gap-3 px-4 pt-3.5 pb-2">
        ...
        <Button
          ...
          size="small"
          shape="circle"
          className="!text-slate-500 hover:!bg-black/5 dark:!text-app-muted dark:hover:!bg-white/10 shrink-0"
          icon={<CloseOutlined />}
      ...
      >
        {data.goals.map((goal) => (
          <div
            key={goal.id}
            className="w-full min-w-full shrink-0 snap-start px-4 pb-3.5 pt-1"
          >
            <Link
              href="/goals"
              className={cn(
                "block w-full rounded-2xl px-3.5 py-3 no-underline transition-colors active:scale-[0.99]",
                // Light: white chip on tinted shell
                "bg-white text-slate-900 shadow-[0_6px_18px_rgba(15,23,42,0.08)] ring-1 ring-slate-900/6",
                "hover:shadow-[0_8px_22px_rgba(15,23,42,0.12)]",
                // Dark: use --card solid (no /opacity — CSS vars break Tailwind alpha)
                "dark:bg-[color-mix(in_srgb,var(--card)_92%,#000)] dark:text-app-fg",
                "dark:shadow-none dark:ring-1 dark:ring-white/10",
                "dark:hover:bg-[color-mix(in_srgb,var(--card)_100%,#000)]"
              )}
            >
